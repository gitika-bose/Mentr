class SessionsController < ApplicationController
  ...
  def show
    @sessions_as_mentor = Session.joins(:mentor).where("mentors.user_id = ? and status < 2", current_user.id).all
    @sessions_as_mentee = Session.joins(:mentee).where("mentees.user_id = ? and status < 2", current_user.id).all
  end
